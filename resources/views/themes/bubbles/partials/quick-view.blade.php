<div class="grid gap-6 sm:grid-cols-2">
    <div class="overflow-hidden rounded-2xl bg-[color:var(--cream)]">
        <img src="{{ $product->cover_image ?? $shop->hero_url }}" alt="{{ $product->t('name') }}" class="aspect-square w-full object-cover">
    </div>
    <div>
        <h3 class="font-display text-2xl font-extrabold text-[color:var(--berry)]">{{ $product->t('name') }}</h3>
        @if ($product->t('short_description'))
            <p class="mt-2 text-sm text-[color:var(--muted)]">{{ $product->t('short_description') }}</p>
        @endif
        <p class="mt-4 text-xl font-extrabold text-[color:var(--rose)]">{{ money($product->current_price) }}</p>
        ...
        <a href="{{ shop_url('products.show', $product->slug) }}" class="mt-4 inline-block text-sm font-bold text-[color:var(--rose)]">{{ __('View details') }}</a>
    </div>
</div>
