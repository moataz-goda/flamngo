...
<article class="bubble-card group relative" data-tilt>
    <span class="gloss"></span>
    <form action="{{ route('wishlist.toggle') }}" method="POST" class="absolute end-3 top-3 z-10">
        @csrf
        <input type="hidden" name="product_id" value="{{ $product->id }}">
        <button type="submit" class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-[color:var(--rose)] shadow-sm ring-1 ring-[color:var(--blush)]" aria-label="{{ __('Wishlist') }}">
    ...
    <a href="{{ shop_url('products.show', $product->slug) }}" class="block" data-cursor="view">
        <div class="img-zoom aspect-[4/5] bg-[color:var(--cream)]">
            ...
            @else
                <div class="flex h-full items-center justify-center bg-gradient-to-br from-[color:var(--berry)] to-[color:var(--rose)] text-white">
                    <span class="font-display text-xl font-bold">Bubbles</span>
                </div>
            @endif
        </div>
        <div class="space-y-2 p-4">
            ...
            @endif
            <h3 class="font-display text-lg font-extrabold text-[color:var(--berry)] transition group-hover:text-[color:var(--rose)]">{{ $product->t('name') }}</h3>
            <div class="flex items-center justify-between gap-2">
                <div class="flex items-baseline gap-2">
                    <span class="text-base font-bold text-[color:var(--rose)]">{{ money($product->current_price) }}</span>
    ...
    <div class="px-4 pb-4">
        <button type="button" data-quick-view="{{ $product->slug }}" class="w-full rounded-full bg-[color:var(--cream)] px-3 py-2 text-xs font-bold text-[color:var(--rose)] ring-1 ring-[color:var(--blush)] transition hover:bg-white">
            {{ __('Quick view') }}
        </button>
    </div>
</article>
