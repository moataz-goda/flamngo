...
<head>
    ...
    <title>@yield('title', $shop->name) — {{ __('Gift Shop') }}</title>
    <meta name="description" content="@yield('meta_description', $shop->t('tagline'))">
    <link rel="icon" href="{{ $shop->logo_url }}">
    @vite(['resources/css/themes/bubbles.css', 'resources/js/themes/bubbles.js'])
    @if ($shop)
        <style>
            :root {
...
<body class="min-h-screen overflow-x-hidden">
    @include('themes.bubbles.partials.header')
    ...
    @include('themes.bubbles.partials.footer')
    ...
    <div id="quick-view-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-4" aria-hidden="true">
        <div class="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
            <button type="button" id="quick-view-close" class="absolute end-4 top-4 rounded-full bg-[color:var(--cream)] px-3 py-1 text-sm font-bold text-[color:var(--berry)]">{{ __('Close') }}</button>
