...
<section class="mx-auto max-w-5xl px-4 py-12">
    ...
    @else
        <div class="reveal space-y-4">
            @foreach ($items as $item)
                <div class="flex flex-wrap items-center gap-4 rounded-[1.5rem] bg-white p-4 ring-1 ring-[color:var(--blush)]/70">
                    <img src="{{ $item['product']->cover_image ?? $shop->hero_url }}" alt="" class="h-20 w-20 rounded-2xl object-cover">
                    <div class="min-w-0 flex-1">
                        <a href="{{ shop_url('products.show', $item['product']->slug) }}" class="font-display text-lg font-extrabold text-[color:var(--berry)]">{{ $item['product']->t('name') }}</a>
                        @if (!empty($item['variant']))
                            <p class="text-sm text-[color:var(--rose)]">{{ $item['variant']->label }}</p>
                        @endif
                    ...
                    <form action="{{ route('cart.update', $item['key']) }}" method="POST" class="flex items-center gap-2">
                        ...
                        <button class="text-sm font-bold text-[color:var(--rose)]">{{ __('Update') }}</button>
                    </form>
                    <div class="font-bold text-[color:var(--rose)]">{{ money($item['line_total']) }}</div>
        ...
        <div class="reveal mt-8 flex flex-wrap items-center justify-between gap-4 rounded-[1.5rem] bg-gradient-to-l from-[color:var(--berry)] to-[color:var(--rose)] p-6 text-white">
            <div>
                <p class="text-white/70">{{ __('Total') }}</p>
                <p class="font-display text-3xl font-extrabold">{{ money($total) }}</p>
            </div>
            <a href="{{ shop_url('reservation.create') }}" class="rounded-full bg-white px-6 py-3 text-sm font-bold text-[color:var(--rose)]" data-magnetic>{{ __('Continue reservation') }}</a>
        </div>
    @endif
</section>
@endsection
