    ...
    <div class="grid gap-10 lg:grid-cols-2">
        <div class="reveal space-y-3">
            <div class="img-zoom overflow-hidden rounded-[2rem] bg-white ring-1 ring-[color:var(--blush)]" data-cursor="view">
                <img id="main-image" src="{{ $product->cover_image ?? $shop->hero_url ?? asset('images/brands/flamingo/hero.jpeg') }}" alt="{{ $product->t('name') }}" class="aspect-square w-full object-cover">
            </div>
        ...
        <div class="reveal">
            ...
            <div class="mt-4 flex flex-wrap items-start justify-between gap-3">
                <h1 class="font-display text-4xl font-extrabold text-[color:var(--plum)]">{{ $product->t('name') }}</h1>
                <form action="{{ route('wishlist.toggle') }}" method="POST">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                    <button type="submit" class="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-bold text-[color:var(--violet)] ring-1 ring-[color:var(--blush)]">
                        ♥ {{ __('Wishlist') }}
                    </button>
                </form>
            </div>
            ...
            <div class="mt-6 flex items-baseline gap-3">
                <span class="text-3xl font-extrabold text-[color:var(--deep-purple)]">{{ money($product->current_price) }}</span>
            ...
            @if ($product->is_available)
                <form action="{{ route('cart.store') }}" method="POST" class="mt-8 space-y-4">
                    ...
                    @if ($product->hasVariants())
                        <div>
                            <label class="mb-2 block text-sm font-bold text-[color:var(--plum)]">{{ __('Choose option') }}</label>
                            <select name="variant_id" required class="w-full max-w-sm rounded-full border border-[color:var(--blush)] px-4 py-2 outline-none focus:ring-2 focus:ring-purple-200">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <label class="text-sm font-bold text-[color:var(--plum)]">{{ __('Quantity') }}</label>
                        <input type="number" name="quantity" value="1" min="1" max="{{ $product->available_quantity }}" class="w-24 rounded-full border border-[color:var(--blush)] px-4 py-2 text-center outline-none focus:ring-2 focus:ring-purple-200">
        ...
        <div class="mt-16">
            ...
            <div class="grid grid-cols-2 gap-5 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($related as $item)
                    @include('themes.flamingo.partials.product-card', ['product' => $item])
                @endforeach
            </div>
        </div>
    @endif
